...
void fft2D(int direction, int width, int height, double **re, double **im) {
    ...
    tempRe = allocDoubleArr2D(height, width);
    tempIm = allocDoubleArr2D(height, width);

    if (direction == FORWARD) {
        //1D FFT on each row (real to complex)
        for (int i = 0; i < height; i++) {
            fft1D(FORWARD, width, re[i], im[i]);
        ...
        //1D FFT on each column resulting from the previous operation (complex to complex)
        for (int i = 0; i < width; i++) {
            fft1D(FORWARD, height, tempRe[i], tempIm[i]);
        ...
        //1D FFT on each column resulting from the previous operation (complex to complex)
        for (int i = 0; i < width; i++) {
            fft1D(REVERSE, height, tempRe[i], tempIm[i]);
            for (int j = 0; j < height; j++) {
                re[j][i] = tempRe[i][j];
                im[j][i] = tempIm[i][j];
            }
    ...
    freeDoubleArr2D(tempRe);
    freeDoubleArr2D(tempIm);
}
